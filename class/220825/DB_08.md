...
for movie in movies.order_by('-opening_date'):
    print(movie.director.name, movie.genre.title, movie.title, movie.opening_date, movie.running_time, movie.screening)
```
...
```python
movies = Movie.objects.all().order_by('-opening_date')[0]
...
```python
for movie in Movie.objects.filter(screening=True).order_by('opening_date'):
    print(movie.director.name, movie.genre.title, movie.title, movie.opening_date, movie.running_time, movie.screening)
```
